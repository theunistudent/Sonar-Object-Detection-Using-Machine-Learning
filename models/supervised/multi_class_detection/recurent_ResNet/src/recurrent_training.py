...
from recurrent_data import SonarImages, SonarVideo, CombinedSonarDataset
from recurrent_model import ResNetLSTMModel
from recurrent_metrics import plot_training_metrics, plot_confusion_matrix_values

# Initialize configparser
config = configparser.ConfigParser()
config.read('/mnt/sda2/liam/SONAR_thesis/models/supervised/multi_class_detection/recurent_ResNet/src/config.ini')
...
model = ResNetLSTMModel(1080, 1920, class_list).to(device=device)
...
# Storage for Confussion Matrix
confusion_matrix_history = pd.DataFrame(columns=['Epoch', 'TP', 'FP', 'FN', 'TN'])
...
def train(dataloader, model, loss_fn, optimizer):
    ...
    scaler = torch.amp.GradScaler('cude')  # Initialize mixed precision scaler
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        with torch.amp.autocast(device_type = 'cuda'):
            ...
            # Expand class weights to match the shape of y for element-wise weighting
            weights = fixed_class_weights.unsqueeze(0).expand_as(y)  # Shape should match (batch_size, num_classes)
            loss = nn.functional.binary_cross_entropy_with_logits(pred, y, weight=weights)
...
# Updated test function (remains the same as before)
...
for t in range(epochs):
    ...
    # Extract and flatten confusion matrix values: [TP, FP, FN, TN]
    TP, FP = conf_matrix[0][0], conf_matrix[0][1]
    FN, TN = conf_matrix[1][0], conf_matrix[1][1]

    # Append current epoch's confusion matrix values to the cumulative DataFrame

# Define the new row for confusion matrix history based on current metrics
...
# Concatenate the new row to the existing confusion_matrix_history DataFrame
confusion_matrix_history = pd.concat([confusion_matrix_history, new_row], ignore_index=True)

# Save cumulative confusion matrix history to a single CSV file
confusion_matrix_csv_path = os.path.join(relative_checkpoint_dir, "confusion_matrix_history.csv")
confusion_matrix_history.to_csv(confusion_matrix_csv_path, index=False)

# Plot the metrics
total_epochs = np.arange(1, len(metrics) + 1)  # Assuming each row in the CSV represents an epoch
plot_training_metrics(metrics, df)

# Call the function to plot confusion matrix values
confusion_matrix_values = confusion_matrix_history[['TP', 'FP', 'FN', 'TN']].to_numpy()
plot_confusion_matrix_values(total_epochs, confusion_matrix_values)
